...
ALERTS = [
    {
        ...
        "window_s":  30,   # 30s in lab = 30min in prod
    ...
    },
    {
        ...
        "window_s":  10,   # 10s in lab = 5min in prod
    ...
    },
    {
        ...
        "window_s":  15,   # 15s in lab = 15min in prod
...
def eval_conditions(metrics: dict, prev_metrics: dict | None, baseline_cost: float) -> dict[str, tuple[bool, str]]:
    ...
    # 2. High error rate (cumulative from metrics)
    ...
    # 3. Cost budget spike (avg_cost_usd > 2x baseline)
    avg_cost  = metrics["avg_cost_usd"]
    threshold = baseline_cost * 2 if baseline_cost > 0 else 9999
    results["cost_budget_spike"] = (
        avg_cost > threshold,
        f"avg_cost=${avg_cost:.5f} (2x baseline=${threshold:.5f})"
    )
    ...
    # 5. Tool failure spike: RuntimeError delta since last poll
    ...
    with httpx.Client(timeout=5.0) as client:
        while True:
            ...
            try:
                ...
                print(f"{RED}[{t}] Cannot reach {base_url}/metrics -- {exc}{RESET}")
                time.sleep(interval)
                continue

            # Set baseline cost on first successful poll with traffic
            ...
            history.append((now_mono, {k: v[0] for k, v in results.items()}))

            # Prune old history
            max_window = max(a["window_s"] for a in ALERTS)
            while history and (now_mono - history[0][0]) > max_window + interval:
                history.popleft()

            # Evaluate each alert with its time window
            for alert in ALERTS:
                name  = alert["name"]
                ...
                # Fire when condition sustained for >= half the window readings
                ...
                if sustained and name not in firing:
                    firing.add(name)
                    print_alert(name, alert["severity"], value_str,
                                alert["condition"], alert["runbook"])
                    log_alert(name, alert["severity"], value_str,
                              alert["condition"], alert["runbook"])
                    ...
                    tag    = f"{RED}[BREACH]{RESET}" if is_now else f"{GREEN}[OK]    {RESET}"
                    print(f"  {tag}  {name:<32}  {value_str}")
